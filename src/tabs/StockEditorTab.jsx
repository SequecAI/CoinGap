...
export default function StockEditorTab({ stockData }) {
  const {
    ...
    per, pbr, eps, bps, dividendYield, marketCap, foreignRate, high52w, low52w,
    kospiPrice, kospiChange, kosdaqPrice, kosdaqChange,
    dayCandles
  } = stockData;
  ...
  // ── 변수 데이터 도출 ──
  const safeChangeRate = num(changeRate);
  const todayCandle = dayCandles && dayCandles.length > 0 ? dayCandles[dayCandles.length - 1] : {};
  const todayOpen = todayCandle.opening_price || currentPrice || 0;
  const stockMomentum = todayOpen > 0 ? ((currentPrice - todayOpen) / todayOpen) * 100 : 0;
  const prevCandle = (dayCandles && dayCandles.length >= 2)
    ? dayCandles[dayCandles.length - 2]
    : {};
  ...
  const variableGroups = [
    {
      ...
      items: [
        { label: '현재가', value: 'STOCK_PRICE', data: currentPrice || 0 },
        { label: '등락률(%)', value: 'STOCK_RATE', data: safeChangeRate },
        { label: '오늘 모멘텀(%)', value: 'STOCK_MOMENTUM', data: stockMomentum },
        { label: '거래량', value: 'STOCK_VOLUME', data: num(volume) },
      ]
    },
    {
      id: 'index',
      ...
      items: [
        { label: 'KOSPI', value: 'KOSPI_PRICE', data: num(kospiPrice) },
        { label: 'KOSPI 등락률(%)', value: 'KOSPI_RATE', data: num(kospiChange) },
        { label: 'KOSDAQ', value: 'KOSDAQ_PRICE', data: num(kosdaqPrice) },
        { label: 'KOSDAQ 등락률(%)', value: 'KOSDAQ_RATE', data: num(kosdaqChange) },
      ]
    },
    {
      id: 'prev',
      ...
      items: [
        { label: '전일 시가(O)', value: 'PREV_O', data: prevCandle.opening_price || 0 },
        { label: '전일 고가(H)', value: 'PREV_H', data: prevCandle.high_price || 0 },
        { label: '전일 저가(L)', value: 'PREV_L', data: prevCandle.low_price || 0 },
        { label: '전일 종가(C)', value: 'PREV_C', data: prevCandle.trade_price || 0 },
        { label: '전일 거래량(V)', value: 'PREV_V', data: prevCandle.candle_acc_trade_volume || 0 },
      ]
    },
    {
      id: 'tech',
      title: '기술 지표 (Technical, 일봉)',
      colorClass: 'bg-teal-50 border-teal-200 hover:border-teal-400 text-teal-600',
      items: [
    ...
    },
    {
      ...
      warning: '※ 재무·외국인·52주 변수는 과거 데이터가 제공되지 않아, 사용 시 백테스트가 비활성화됩니다.',
      items: [
        { label: 'PER', value: 'PER', data: num(per) },
        { label: 'PBR', value: 'PBR', data: num(pbr) },
        { label: 'EPS', value: 'EPS', data: num(eps) },
        { label: 'BPS', value: 'BPS', data: num(bps) },
        { label: '배당수익률(%)', value: 'DIVIDEND_YIELD', data: num(dividendYield) },
        { label: '시가총액', value: 'MARKET_CAP', data: num(marketCap) },
        { label: '외국인비율(%)', value: 'FOREIGN_RATE', data: num(foreignRate) },
        { label: '52주 고가', value: 'HIGH_52W', data: num(high52w) },
        { label: '52주 저가', value: 'LOW_52W', data: num(low52w) },
      ]
    },
    {
      id: 'math',
  ...
  const handleRunBacktest = async () => {
    if (usesUnbacktestableStockVars(formula)) return;
    ...
    try {
      ...
      setBacktestResult({ ...result, stockName });
    } catch (e) {
  ...
  const formatBacktestPeriod = (bt) => {
    ...
    const name = bt.stockName || bt.stockCode || '';
    return `${name} 백테스트 ${bt.days}거래일(${toShort(bt.periodStart)}~${toShort(bt.periodEnd)})`;
  };
    ...
    <div className="space-y-6">
            ...
            {indicators.map(ind => {
              ...
              return (
                <div
                  ...
                  className={`bg-slate-900 text-white p-5 rounded-[2rem] shadow-xl border-2 cursor-pointer transition-all relative overflow-hidden ${isSelected ? 'border-emerald-400 shadow-emerald-900/30' : 'border-slate-800 hover:border-emerald-500/40'}`}
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-600/10 rounded-full blur-[60px]"></div>
                  <div className="relative z-10 space-y-3">
                    ...
                    {ind.backtest && ind.backtest.stockCode === stockCode ? (
            ...
            {indicators.map(ind => (
              <div
                ...
                className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center group
                  ${ind.id === selectedSavedIndicatorId ? 'border-emerald-500 bg-emerald-50 shadow-md' : 'border-slate-100 bg-white hover:border-emerald-300'}`}
              >
      ...
      <div ref={editorRef} className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="border-b border-slate-100 pb-6">
          <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2 flex items-center gap-1.5 ml-1">
            <Code size={14} /> Indicator Name <Pencil size={12} className="text-emerald-300 ml-1" />
          </label>
          <input
            className="w-full text-2xl md:text-3xl font-black text-slate-900 outline-none placeholder-slate-300 bg-transparent transition-all focus:text-emerald-600 px-1"
            value={indicatorName}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {variableGroups.map(group => (
                <div key={group.id} className="bg-slate-50 p-4 rounded-3xl border border-slate-100 transition-all">
                  ...
                  >
                    <p className="text-[10px] font-black text-slate-400 uppercase ml-1 group-hover/toggle:text-emerald-500 transition-colors">
                      {group.title}
                    </p>
                    {openGroups[group.id]
                      ? <ChevronUp size={16} className="text-slate-400 group-hover/toggle:text-emerald-500 transition-colors" />
                      : <ChevronDown size={16} className="text-slate-400 group-hover/toggle:text-emerald-500 transition-colors" />}
                  </div>
        ...
        <div className="space-y-4 pt-6 border-t border-slate-100">
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <textarea
                ref={formulaInputRef}
                className="w-full h-32 md:h-40 bg-slate-900 text-emerald-400 font-mono text-base p-4 pl-20 md:p-6 md:pl-28 rounded-3xl outline-none focus:ring-4 focus:ring-emerald-500/20 transition-all resize-none leading-relaxed"
                value={formula}
          ...
          <div className="space-y-3 pt-4">
            ...
            <div className="flex items-stretch gap-1 md:gap-1.5">
              ...
              <input
                ...
                className="w-12 md:w-16 shrink-0 bg-white border border-slate-300 rounded-lg px-1 py-1 text-center font-black text-xs tabular-nums text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400"
        ...
        <div className="flex flex-col md:flex-row gap-4 pt-6 border-t border-slate-100">
          <div className={`flex-1 p-6 md:p-8 rounded-[2.5rem] border-2 transition-all relative overflow-hidden ${currentResult.error ? 'bg-slate-50 border-dashed border-slate-300' : 'bg-white border-emerald-100 shadow-xl shadow-emerald-50'}`}>
            {!currentResult.error ? (
              <div className="relative z-10 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="p-2 bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-100"><Play size={18} /></div>
          ...
          <div className="md:w-48 flex flex-col gap-2">
            <button
              ...
              className="h-full min-h-[120px] bg-emerald-600 hover:bg-emerald-700 text-white rounded-[2.5rem] font-black text-sm shadow-xl shadow-emerald-200 transition-all flex flex-col items-center justify-center gap-3 p-6 active:scale-95"
            >
              <Save size={24} />
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-4">
        ...
        <p className="text-[11px] font-medium text-slate-500 leading-relaxed">
          ※ 백테스트 결과는 과거 <strong>일봉</strong> 데이터에 기반한 <strong className="text-rose-500 font-bold">참고용 지표</strong>입니다. <strong className="text-rose-500 font-bold">미래 수익을 보장하지 않으며</strong>, 시장 환경·유동성·체결 미끄럼 등은 반영되지 않습니다. 단일 결과만으로 매매 결정을 내리지 마세요.
        </p>
        ...
        {usesUnbacktestableStockVars(formula) ? (
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 flex items-start gap-3">
            <AlertCircle size={18} className="text-rose-500 shrink-0 mt-0.5" />
            <p className="text-xs font-bold text-rose-600 leading-relaxed">
              재무·시장 스냅샷 변수(<code className="font-mono">PER, PBR, EPS, BPS, DIVIDEND_YIELD, MARKET_CAP, FOREIGN_RATE, HIGH_52W, LOW_52W</code>)가 포함된 수식은 과거 데이터가 없어 백테스트가 비활성화됩니다. 해당 변수를 제거하면 사용 가능합니다.
            </p>
          </div>
        ) : backtestRunning ? (
          <div className="space-y-2">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 transition-all duration-200" style={{ width: `${backtestProgress * 100}%` }}></div>
            </div>
          </div>
        ) : backtestResult && backtestResult.stockCode === stockCode ? (
          <div className="space-y-4">
            ...
            {backtestResult && backtestResult.stockCode !== stockCode && (
              <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-center">
                <p className="text-xs font-bold text-slate-600">
                  현재 종목(<strong className="text-emerald-600">{stockName}</strong>)에 대한 백테스트 결과가 없습니다.
                </p>
              </div>
            )}
            <button
              onClick={handleRunBacktest}
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-sm shadow-lg shadow-emerald-200 transition-all flex items-center justify-center gap-2 active:scale-95"
            >
              <Play size={18} />
              백테스트 실행 ({stockName} 일봉 60거래일)
            </button>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] p-6 max-w-sm w-full shadow-2xl space-y-6">
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-2xl ${modalState.type === 'alert' ? 'bg-orange-100 text-orange-600' : 'bg-emerald-100 text-emerald-600'}`}>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black shadow-lg shadow-emerald-200 transition-all active:scale-95"
              >
                확인
              </button>
            </div>
