...
function TradeRow({ trade }) {
  // 라벨은 손익 부호 기준. 신호 종류(Safety_TP / Hard_SL)는 작은 회색 서브 라벨.
  ...
  const reasonHint = !enter ? signalSub(trade) : null;
...
function signalSub(trade) {
  if (trade.action === 'EXIT_TP') return '익절 신호';
  if (trade.action === 'EXIT_SL') return '손절 신호';
  return null;
}
